...
unless((-e "hlr/$trid_login_hlrname") && ($hlrclass eq "clasa2" )) #if does not exist or exists for a different class #CUSTOM
{
...
#ACTION: check the clearance level: (tipcont==TRAINING && clasa==any)||(tipcont==II && ultima_clasa_promovata==0)
unless($tipcont == 0 || $tipcont == 2 && $ultimaclasa == 0) #CUSTOM
{
#ACTION: close all resources
truncate(transactionFILE,0);
seek(transactionFILE,0,0);				#go to beginning of transactionfile
