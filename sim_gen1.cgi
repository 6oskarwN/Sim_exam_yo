...
open(transactionFILE,"+< sim_transaction") || dienice("ERR01_op",1,\"$! $^E $?");
...
close(transactionFILE) || dienice("ERR02_cl",1,\"$! $^E $?");
...
unless($heximac eq $pairs[7]) { dienice("ERR01",1,\"transaction_id sha1 mismatch: $get_trid");}

#check case 1, timestamp
elsif (timestamp_expired($pairs[1],$pairs[2],$pairs[3],$pairs[4],$pairs[5],$pairs[6])>0) { 
                                             dienice("ERR02",0,\"transaction timestamp expired"); } #normally not logged
...
open(userFILE,"< sim_users") || dienice("ERR01_op",1,\"$! $^E $?");
...
if(-e "hlr/$trid_login_hlrname")
 {
open(HLRread,"< hlr/$trid_login_hlrname") || dienice("ERR01_op",1,\"$! $^E $?"); #open for reading only
...
unless((-e "hlr/$trid_login_hlrname") && ($hlrclass eq "clasa1" )) #if does not exist or exists for a different class #CUSTOM
{
...
close(userFILE) || dienice("ERR02_cl",1,\"$! $^E $?");
...
#ACTION: check the clearance level: (tipcont==TRAINING && clasa==any)||(tipcont==I && ultima_clasa_promovata==0)
unless($tipcont == 0 || $tipcont == 1 && $ultimaclasa == 0) #CUSTOM
{
#ACTION: close all resources
truncate(transactionFILE,0);
seek(transactionFILE,0,0);				#go to beginning of transactionfile
...
##BLOCK: Generate new transaction: EXAM I and close transaction file
#{
...
#CUSTOM: pagecode=4 pentru exam cl I
my $entry = "$hexi $trid_login 4 $exp_sec $exp_min $exp_hour $exp_day $exp_month $exp_year";
...
my @database=("db_tech1","db_ntsm","db_op1","db_legis1");       #CUSTOM set the name of used databases and their order
...
my @strips=("strip_db_tech1","strip_db_ntsm","strip_db_op1","strip_db_legis1");#CUSTOM
